...
class GMAE_node(pl.LightningModule):
    ...
    def compute_pos_embeddings(self, batched_data):
        ...
        # self.spatial_pos_encoder = self.spatial_pos_encoder.to('cuda:0')
        # spatial_pos = spatial_pos.to('cuda:0')
        # graph_attn_bias = graph_attn_bias.to('cuda:0')
        # attn_bias = attn_bias.to('cuda:0')
    ...
    def generate_pretrain_embeddings_for_downstream_task2(self, batched_data):
        graph_node_feature, graph_attn_bias = self.compute_pos_embeddings(batched_data)
        output = self.encoder(graph_node_feature, graph_attn_bias)
        output = output  # [n_graph(n_central_node), n_feature]
        return output
    ...
    #testing step for testing the model
    def test_step(self, batched_data, batch_idx):
        ...
        # Calculate accuracy by the end of decoder
        acc = cosine_similarity(y_hat, y_gt, dim=-1).mean()
        #print('acc', acc)
        self.log('test_loss', loss, batch_size=1,prog_bar=True)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--multi_hop_max_dist', type=int, default=0)
        parser.add_argument('--default_root_dir', type=str, default='exps/computers')
        parser.add_argument('--reload_dataloaders_every_epoch', type=int, default=1)
    ...
    @staticmethod
    def init_model(args, dm, n_node_features):
        model = GMAE_node(
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_bias=None):
        ...
        q = q * self.scale
        #print('here 1->',attn_bias.is_cuda)

        x = torch.matmul(q, k)  # [b, h, q_len, k_len]
        if attn_bias is not None:
            #print('here 2->',attn_bias.is_cuda)
            #print('here 3 ->',x.is_cuda)
            x = x + attn_bias
