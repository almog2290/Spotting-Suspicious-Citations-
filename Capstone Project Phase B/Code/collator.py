...
class Batch():
    def __init__(self, min_node_num, attn_bias, spatial_pos, in_degree, out_degree, x, y ,node_id):
...
def collator(items, spatial_pos_max=20):
    ...
    items = [(item[0], item[1], item[2], item[3], item[4], item[5], item[6] ,item[7]) for item in items]
    xs, ys, adjs, attn_biases, spatial_poses, in_degrees, out_degrees , node_id = zip(*items)
