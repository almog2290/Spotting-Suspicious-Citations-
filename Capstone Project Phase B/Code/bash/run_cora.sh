...
[ -z "${seed}" ] && seed="1"
[ -z "${arch}" ] && arch="--peak_lr 1e-4 --end_lr 1e-9 --l1 0 --l2 5 --batch_size 32 --ffn_dim 64 --hidden_dim 64 --num_heads 8 --dropout_rate 0.5"
[ -z "${mask_ratio}" ] && mask_ratio="0.8"
[ -z "${warmup_updates}" ] && warmup_updates="40000"
[ -z "${tot_updates}" ] && tot_updates="400000"
[ -z "${n_encoder_layers}"] && n_encoder_layers="4"
[ -z "${n_decoder_layers}"] && n_decoder_layers="2"
[ -z "${dataset_name}"] && dataset_name="Cora"
...
python entry.py --num_workers 4 --seed $seed \
